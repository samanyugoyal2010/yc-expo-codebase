...
// fail maps domain errors onto status codes. A stale receipt is a 409 rather
// than a 500: it means someone else owns the lease now, which is an expected
// outcome of at-least-once delivery, not a server fault.
...
// queueOp dispatches /v1/queues/{name}[/{action}].
...
func (s *Server) lease(w http.ResponseWriter, r *http.Request, q *queue.Queue) {
	...
	// Long polling is capped so a client cannot hold a connection open forever.
